...
export const useDataStore = defineStore('data', () => {
  ...
  const subscriptions: Ref<Map<string, $Subs>>
    = ref(new Map<string, $Subs>())
  ...
  */
  ...
  /* set$Subs() : enregistrement en phase 0 de la souscription d'une organisation */
  const set$Subs = (org: string, subscription: $Subs) => {
    subscriptions.set(org, subscription)
  }

  /* getOrgs$Subs() : retourne la liste des organisations ayant une souscription */
  const getOrgs$Subs = () : string[] => {
    return Array.from(subscriptions.keys())
  }

  /* get$Subs() : retourne un clone de la subscription enregistrée en vue d'édition */
  const get$Subs = (org: string) : $Subs | null => {
    const s = subscriptions.get(org) || null
    return !s ? null : $Subs.fromSerial(s.serial())
  }
  ...
  ******************************************************************************/
  ...
  return { 
    ...
    set$Subs, get$Subs, getOrgs$Subs,
