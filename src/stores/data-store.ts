...
export const useDataStore = defineStore('data', () => {
  ...
  const documents: Ref<Map<string, Map<string, Map<string, Document>>>> = ref(new Map<string, Map<string, Map<string, Document>>>())
  ...
  */
  const setDoc = (org: string, doc: Document) : Document => {
    ...
    const d = ecl.get(doc._pk)
    if (d && d.v >= doc.v) return d
    if (doc.deleted) {
      if (d) ecl.delete(doc._pk)
  ...
  /* Retourne le document dans la stucture ou null s'il n'y est pas */
  const getDoc = (org: string, clazz: string, pk: string) : Document => {
    let eorg = documents.value.get(org)
    if (!eorg) return null
    let ecl = eorg.get(clazz)
    if (!ecl) return null
  ...
  const getClDocs = (org: string, clazz: string)
    ...
    let ecl = eorg.get(clazz)
    if (!ecl) return r
    for(const [, doc] of ecl) r.push(doc)
    return r
  }
  ...
  */
  const getColl = (org: string, clazz: string, colName: string, colValue: string)
    ...
    let ecl = eorg.get(clazz)
    if (!ecl) return r
    for(const [, doc] of ecl) {
      const s = doc.propertyAsSet(colName)
      if (s.has(colValue)) r.push(doc)
    }
    return r
  }
  ...
  */
  ...
  return { 
    setCpt, cpt,
    setDoc, getDoc, deleteDoc, getColl, getClDocs, getOrgs, setDef, deleteDef,
    subs0, subs1, subs2, subs2N, subs2NV, hasSubs, 
    queueForSync, startSyncQueue,
